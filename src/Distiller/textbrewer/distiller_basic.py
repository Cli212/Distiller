...
class BasicDistiller(AbstractDistiller):
        ...
        for current_epoch in tqdm(range(int(num_epochs)), disable=tqdm_disable):
            if self.t_config.q and current_epoch % self.t_config.num_reaug == 0 and current_epoch != 0:
                ...
                if self.local_rank not in [-1, 0]:
                    while True:
                        try:
                            train_dataset = torch.load(os.path.join(self.t_config.output_dir,f'train_dataset_{current_epoch}.bin'))
                            break
                        except:
                            continue
                    # torch.distributed.barrier()
                    ...
                    while count < QUEUE_LIMIT:
                        try:
                            ...
                            torch.save(train_dataset, os.path.join(self.t_config.output_dir,f'train_dataset_{current_epoch}.bin'))
                            break
                        except queue.Empty:
    ...
    def train_on_batch(self, batch, args, s_batch=None):
        if self.d_config.is_caching_logits is False:
            ...
            _, (student_batch, results_S) = get_outputs_from_batch(batch, self.t_config.device, self.model_T,
                                                                   self.model_S, self.local_rank, args,
                                                                   self.t_config.mixup,
                                                                   task_type=self.t_config.task_type,
    ...
    def augment_data(self, batch):
        finetuning_task = self.model_T.module.module.config.finetuning_task if hasattr(self.model_T.module,
                                                                                       "module") else self.model_T.module.config.finetuning_task
        new_batch = []
        labels = []
        for i in range(self.t_config.repeated_aug - 1):
            ...
            del tmp_batch
        if self.d_config.soft_label_weight > 0.0:
            labels = torch.LongTensor(labels).to(self.model_T.module.module.device if hasattr(self.model_T.module,
                                                                                              "module") else self.model_T.module.device)
            ...
            inputs = {key: value.to(self.model_T.module.module.device if hasattr(self.model_T.module,
                                                                                 "module") else self.model_T.module.device)
                      for key, value in inputs.items()}
            outputs = self.model_T(**inputs, labels=labels)
            predictions = outputs.logits.detach().cpu()
            if finetuning_task not in ["stsb","cloth"]:
