...
def get_outputs_from_batch(batch, device, model_T, model_S, args, no_teacher_forward=False):
    ...
    if mixup:
        ...
        args.pop("mixup")
        mixup_value = args.pop("mixup_value")
        if type(batch) is dict:
            if 'teacher' in batch and 'student' in batch and 'teacher' in mixup_batch and 'student' in mixup_batch:
                teacher_batch = batch['teacher']
                student_batch = batch['student']
                teacher_mixup_batch = mixup_batch['teacher']
                student_mixup_batch = mixup_batch['student']
                # teacher_batch = move_to_device(teacher_batch, device)
                # teacher_mixup_batch = move_to_device(teacher_mixup_batch, device)
                #teacher outputs
                if no_teacher_forward is True:
                    ...
                    if 'teacher_cache' in batch:
                        ...
                        with torch.no_grad():
                            teacher_batch = mixup_embeds(model_T,teacher_batch,teacher_mixup_batch,mixup_value,local_rank)
                ...
                # student_mixup_batch = move_to_device(student_mixup_batch, device)
                student_batch = mixup_embeds(model_S,student_batch,student_mixup_batch,mixup_value,local_rank)
                ...
                mixup_batch = move_to_device(mixup_batch, device)
                if no_teacher_forward is True:
                    results_T = None
                else:
                    with torch.no_grad():
