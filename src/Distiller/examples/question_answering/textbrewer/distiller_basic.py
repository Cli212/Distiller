...
class BasicDistiller(AbstractDistiller):
    ...
    def train_with_num_epochs(self, optimizer, scheduler, tqdm_disable, dataloader, max_grad_norm, num_epochs, callback, batch_postprocessor, **args):
        mix_dataloader = args.pop("mix_dataloader")
        ...
        for current_epoch in tqdm(range(int(num_epochs)),disable=tqdm_disable):
            ...
            if mix_dataloader:
                if self.local_rank != -1 and hasattr(mix_dataloader, 'sampler'):
                    mix_dataloader.sampler.set_epoch(current_epoch)
                mixed_dataloader = zip(dataloader, mix_dataloader)
                args['mixup'] = True
                for step, batch in tqdm(enumerate(mixed_dataloader), disable=tqdm_disable):
                    if self.d_config.is_caching_logits is False and batch_postprocessor is not None:
                        batch = batch_postprocessor(batch)
                    total_loss, losses_dict = self.train_on_batch(batch, args.copy())
